function StateManager() {
	...
	this.refresh = function(){
		...
		this.active =null;
		this.set(this.active_name);
		this.ctx = ctx.clearRect(0, 0, canvas.width, canvas.height);
		
		this.get(this.active_name).init();
		this.tick(ctx);
	}
}
...
function Player(data) {
	...
	this.hasWinner = function() {
		...
		if (hasWon(seed)) {
			...
			return oppSeed;
		} if (m.length === 0 && !hasWon(seed) && !hasWon(oppSeed)){
			return true;
		}
		return false;
	}
